class Communication::Ses::ThanksWaiting < Communication::Communication
  ...
  def use_row?(row_hash)
    (!row_hash.nil? and !row_hash[:select].nil? and ( row_hash[:select].strip.length > 0) )
  end
  ...
  def label(d)
    ...
    d.name= 'ThanksWaiting'.underscore.to_sym
    d
